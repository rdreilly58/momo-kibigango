...
- 🔴 08:54 **Telegram credentials successfully added to OpenClaw config** — Bot token `(stored in ~/.openclaw/config.json)` and Chat ID `(stored in ~/.openclaw/config.json)` configured in `~/.openclaw/config.json`; OpenClaw Gateway restarted; daily briefings now enabled <!-- dc:type=decision dc:importance=7.5 dc:date=2026-04-11 -->
